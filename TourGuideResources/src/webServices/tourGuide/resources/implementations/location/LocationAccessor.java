...
import webServices.tourGuide.resources.implementations.helpful.DatabaseConnection;
...
public class LocationAccessor {
	...
	this.db = DatabaseConnection
		.getConnection("config/DBConfiguration.xml");
    }
     ...
     *            the loc
     */
    public void saveLocation(Location loc, String userId) {
	try {
	    if (!contains(loc.getName())) {
		String query = "INSERT INTO places (name, coordinates link,description, USER_ID) VALUES (?,?,?,?,?,?)";
     ...
     * @return the list
     */
    public List<Location> retrieveLocationByUser(String userId) {
	ResultSet result = null;
	String query = "SELECT * FROM places WHERE USER_ID=?";
	PreparedStatement location = this.db.preparedStatement(query);
	try {
	    location.setString(1, userId);
	    result = location.executeQuery();
	} catch (SQLException e) {
	    e.printStackTrace();
	}
    ...
    public void deleteLocationByUser(String userId) {
	String query = "DELETE FROM places WHERE USER_ID=?";
	PreparedStatement deleteUser = this.db.preparedStatement(query);
	try {
